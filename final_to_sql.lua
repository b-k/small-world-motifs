infile = arg[1]
outdb = arg[2].."/graphs.db"
outtab = arg[3]

sh = function(doit) os.execute(doit) end --save some typing
Qp = function(query) 
    print([[sqlite3 --header ]] .. outdb .. [[ "]].. query ..[["]])
    sh([[sqlite3 --header ]] .. outdb .. [[ "]].. query ..[["]]) end
Q = function(query) sh([[sqlite3 --header  .. db .. [[ "]].. query ..[["]]) end


--push sort-of-human-readable output into a db.
bring_in = function(infile, outdb)
   sh([[ grep For ]]..infile..[[ | sed -e 's/For\t/"/; s/[ \t]\+(α, β, μ)=/"/;' -e 's/\t\([0-9.]\+\)/|\1/g;' -e '1isubgraph|α|β|μ' |\
   apop_text_to_db -d"|" -ea - ]].. outtab .. " " .. outdb)
end
